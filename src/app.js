...
import { composeSequenceDisplayName, createSequence, markSequenceStepStatus, saveSequenceTemplate, truncateSequenceInstanceFromStep } from "./sequences/sequence-engine.js";
import {
...
async function renderPromotionDetail(promotionId) {
  ...
  const promotion = { id: promotionSnapshot.id, ...promotionSnapshot.data() };
  const eventsSnapshot = await getDocs(query(collection(db, "users", currentUser.uid, "events"), where("promotionId", "==", promotionId)));
  const events = eventsSnapshot.docs.map((eventDoc) => ({ id: eventDoc.id, ...eventDoc.data() }));
  const touchpoints = Array.isArray(promotion.touchpoints) ? promotion.touchpoints : [];
  const containerEventsByTouchpointId = events.reduce((acc, event) => {
    if (event.type !== "promotion_touchpoint" || event.leadId) return acc;
    acc.set(event.touchpointId, event);
    return acc;
  }, new Map());

  const touchpointCards = touchpoints.map((touchpoint, index) => {
    const event = containerEventsByTouchpointId.get(touchpoint.id);
    const fallbackDue = toPromotionDate(promotion.endDate)
      ? Timestamp.fromDate(new Date(toPromotionDate(promotion.endDate).getTime() - (Number(touchpoint.offsetDays) || 0) * 86400000))
      : null;
    const dueAt = event?.scheduledFor || fallbackDue;
    const state = event?.status === "skipped" || event?.status === "completed" || event?.completed ? "completed" : "active";
    return {
      id: touchpoint.id,
      order: Number.isInteger(touchpoint.order) ? touchpoint.order : index,
      title: touchpoint.name || `Touchpoint ${index + 1}`,
      subtitle: `Touchpoint ${index + 1} of ${touchpoints.length || 1}`,
      dueAt,
      state,
  ...
  const activeCards = touchpointCards.filter((card) => card.state === "active");
  const completedCards = touchpointCards.filter((card) => card.state === "completed");
  const touchpointEventPathByTouchpointId = new Map(touchpointCards.map((card) => [card.id, card.eventId ? `#promotion-event/${card.eventId}` : ""]));
  const activeIndex = Math.max(0, touchpointCards.findIndex((card) => card.state === "active"));
  const currentIndex = touchpointCards.length
    ? Math.min(touchpointCards.length - 1, activeIndex)
    : 0;
  const promotionProgressMarkup = buildSubEventProgressMarkup({
    entityType: "promotion",
    totalSubEvents: touchpoints.length || 1,
  ...
  const renderTouchpointCards = (cards) => cards.length
    ? cards.map((card) => {
      if (!card.eventId) {
        return `<article class="panel feed-item feed-item--promotion"><p class="feed-type">Promotion</p><h3>${escapeHtml(card.title)}</h3><p>${escapeHtml(card.subtitle)}</p><p><strong>Due:</strong> ${formatDate(card.dueAt)}</p><p class="view-message">Touchpoint event will appear when this promotion syncs.</p></article>`;
      }
      return `<article class="panel feed-item feed-item-clickable feed-item--promotion" data-open-promotion-touchpoint="${card.eventId}" tabindex="0" role="button"><p class="feed-type">Promotion</p><h3>${escapeHtml(card.title)}</h3><p>${escapeHtml(card.subtitle)}</p><p><strong>Due:</strong> ${formatDate(card.dueAt)}</p><div class="button-row"><a class="timeline-link-pill" href="${appendOriginToHash(`#promotion-event/${card.eventId}`, window.location.hash)}">Open</a></div></article>`;
    }).join("")
    : '<p class="view-message">No touchpoints.</p>';

  viewContainer.innerHTML = `<section class="crm-view crm-view--promotions"><div class="view-header"><h2>${escapeHtml(promotion.name || "Promotion")}</h2><div class="view-header-actions"><button id="promotion-back-btn" type="button" class="secondary-btn">Back</button><button id="promotion-edit-btn" type="button" class="secondary-btn">Edit</button></div></div>${promotionProgressMarkup}<div class="panel detail-grid panel--promo-summary"><p><strong>End Date:</strong> ${formatDate(promotion.endDate)}</p><p><strong>Cohort Size:</strong> ${Array.isArray(promotion.leadIds) ? promotion.leadIds.length : 0}</p><p><strong>Status:</strong> ${escapeHtml(promotion.status || "active")}</p></div><div class="panel panel--promo-section"><h3>Active Touchpoints</h3><div class="feed-list">${renderTouchpointCards(activeCards)}</div></div><div class="panel panel--promo-section"><h3>Completed Touchpoints</h3><div class="feed-list">${renderTouchpointCards(completedCards)}</div></div></section>`;
  ...
  viewContainer.querySelectorAll("[data-open-promotion-touchpoint]").forEach((itemEl) => {
    const open = () => {
      const touchpointEventId = itemEl.dataset.openPromotionTouchpoint;
      if (!touchpointEventId) return;
      window.location.hash = appendOriginToHash(`#promotion-event/${touchpointEventId}`, window.location.hash);
    };
...
function buildSequenceStepMarkup(step, index, expandedSections = {}) {
  ...
  return `<article class="panel detail-grid promotion-wizard-message-card" draggable="true" data-sequence-step-card="${index}" data-sequence-step-id="${escapeHtml(step.id)}"><div class="sequence-step-card-header"><button type="button" class="secondary-btn" data-sequence-drag-handle="${index}" aria-label="Drag to reorder">↕</button><p class="promotion-wizard-card-title"><strong>Step ${index + 1}</strong></p></div><input data-sequence-step-primary="${index}" value="${escapeHtml(primaryValue)}" placeholder="What needs to happen?" /><button type="button" class="link-btn sequence-section-toggle" data-sequence-toggle-description="${escapeHtml(step.id)}">${taskNotes ? "Description added" : "Add description"}<span aria-hidden="true">${expandedDescription ? "▴" : "▾"}</span></button><div data-sequence-task-fields="${index}" class="${expandedDescription ? "" : "hidden"}"><label class="full-width">Description<textarea rows="3" data-sequence-step-task-notes="${index}">${escapeHtml(taskNotes)}</textarea></label></div><button type="button" class="link-btn sequence-section-toggle" data-sequence-toggle-email="${escapeHtml(step.id)}">${stepType === "email" ? "Email configured" : "Add Email"}<span aria-hidden="true">${expandedEmail ? "▴" : "▾"}</span></button><div data-sequence-email-fields="${index}" class="${expandedEmail ? "" : "hidden"}"><label>Step Type<select data-sequence-step-type="${index}"><option value="task_reminder" ${stepType === "task_reminder" ? "selected" : ""}>Task Reminder</option><option value="email" ${stepType === "email" ? "selected" : ""}>Email</option></select></label><label>To<input type="email" data-sequence-step-to="${index}" value="${escapeHtml(step.toEmail || "")}" placeholder="person@example.com" /></label><label class="template-checkbox-row"><input type="checkbox" data-sequence-step-use-contact-email="${index}" ${step.useContactEmail ? "checked" : ""} /><span>Use contact email for this step</span></label><label>Subject<input data-sequence-step-subject="${index}" value="${escapeHtml(template.subjectText)}" /></label><label>Intro<input data-sequence-step-intro="${index}" value="${escapeHtml(template.introText)}" /></label><label class="template-checkbox-row"><input type="checkbox" data-sequence-step-populate-name="${index}" ${template.populateName ? "checked" : ""} /><span>Auto populate name</span></label><label>Body<textarea rows="4" data-sequence-step-body="${index}">${escapeHtml(template.bodyText)}</textarea></label><label>Outro<input data-sequence-step-outro="${index}" value="${escapeHtml(template.outroText)}" /></label></div><button type="button" class="link-btn sequence-section-toggle" data-sequence-toggle-details="${escapeHtml(step.id)}">Details<span aria-hidden="true">${expandedDetails ? "▴" : "▾"}</span></button><div data-sequence-details-fields="${index}" class="${expandedDetails ? "" : "hidden"}">${index > 0 ? `<label class="template-checkbox-row"><input type="checkbox" data-sequence-step-trigger-immediate="${index}" ${step.triggerImmediatelyAfterPrevious ? "checked" : ""} /><span>Trigger immediately upon completion of the previous step</span></label><label class="sequence-offset-row"><span>Delay by</span><input class="promotion-wizard-offset-input" type="number" min="0" data-sequence-step-delay="${index}" value="${Number(step.delayDaysFromPrevious) || 0}" ${step.triggerImmediatelyAfterPrevious ? "disabled" : ""} /><span>days after previous step</span></label>` : '<p>Step 1 sends at Start Date (or immediately if unset).</p>'}</div></article>`;
}
...
function applySequenceStepContactRules(step, index, selectedContact = null) {
  ...
  const useContactEmail = useContactCheckbox.checked === true;
  if (useContactEmail) {
    if (hasContact) {
...
async function renderSequenceCreateFlow() {
  ...
  const initialBannerMessage = window.sessionStorage.getItem("sequence-template-banner") || "";
  if (initialBannerMessage) {
    window.sessionStorage.removeItem("sequence-template-banner");
  }
  ...
  const render = () => {
    if (state.page === "select") {
      ...
      viewContainer.innerHTML = `<section class="crm-view crm-view--promotions"><div class="view-header"><h2>Sequence Templates</h2><div class="view-header-actions"><button id="sequence-cancel-btn" type="button" class="secondary-btn">Back</button></div></div>${bannerMarkup}<div class="panel"><h3>Template Library</h3>${options}</div></section>`;
      state.bannerMessage = "";
    ...
    if (state.page === "schedule") {
      ...
      viewContainer.innerHTML = `<section class="crm-view crm-view--promotions"><div class="view-header"><h2>Start Sequence</h2></div><div class="panel detail-grid"><p><strong>${escapeHtml(sequenceLabel)}</strong></p><label>Instance Name (optional)<input id="sequence-instance-name" value="${escapeHtml(state.instanceName)}" placeholder="e.g. Order Number, Company" /></label><label>Add Contact (optional)<select id="sequence-contact-id"><option value="">No contact</option>${contacts.map((c) => `<option value="${c.id}" ${state.contactId === c.id ? "selected" : ""}>${escapeHtml(c.name || c.email || "Unnamed")}</option>`).join("")}</select></label><label>Start Date (optional)<input id="sequence-start-date" type="datetime-local" value="${escapeHtml(state.startDate)}" /></label></div><div class="button-row"><button id="sequence-schedule-back-btn" type="button" class="secondary-btn">Back</button><button id="sequence-edit-btn" type="button" class="secondary-btn">Edit</button><button id="sequence-save-btn" type="button">Start</button></div></section>`;
    ...
    if (state.page === "steps") {
      ...
      viewContainer.innerHTML = `<section class="crm-view crm-view--promotions"><div class="view-header"><h2>${escapeHtml(state.editingTemplateId ? "Edit Template" : "Create Template")}</h2></div>${saveErrorMarkup}<div class="panel detail-grid"><label>Template Name<input id="sequence-name-input" value="${escapeHtml(state.sequenceName || state.selectedTemplate?.name || "")}" placeholder="Untitled Sequence" /></label></div><div class="promotion-wizard-message-list">${state.steps.map((step, index) => buildSequenceStepMarkup(step, index, {
        emailStepId: state.expandedEmailStepId,
        descriptionStepIds: state.expandedDescriptionStepIds,
      ...
      document.querySelectorAll("[data-sequence-toggle-description]").forEach((toggleEl) => {
        toggleEl.addEventListener("click", () => {
          ...
          state.expandedDescriptionStepIds = state.expandedDescriptionStepIds.includes(stepId)
            ...
            : [...state.expandedDescriptionStepIds, stepId];
          render();
        });
      });
      ...
      document.querySelectorAll("[data-sequence-toggle-details]").forEach((toggleEl) => {
        toggleEl.addEventListener("click", () => {
          ...
          state.expandedDetailStepIds = state.expandedDetailStepIds.includes(stepId)
            ...
            : [...state.expandedDetailStepIds, stepId];
          render();
        });
      });
...
async function renderSequenceEventDetail(eventId) {
  ...
  const renderStepCard = (entry, withActions = false) => {
    ...
    const reminderContent = entryType === "task_reminder"
      ? `<div class="detail-grid"><p><strong>Task Name:</strong> ${escapeHtml(String(entry?.taskConfig?.title || entry?.taskTitle || entry.stepName || "Task Reminder"))}</p>${descriptionText ? `<label class="full-width">Description<textarea rows="3" readonly>${escapeHtml(descriptionText)}</textarea></label>` : ""}</div>`
      : "";
